  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-xs">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="font-semibold text-[14px] text-gray-900">Practice Questions</h3>
          {questions.length > 0 && (
            <p className="text-xs text-gray-400 mt-0.5">{answered}/{questions.length} answered</p>
          )}
        </div>
        <button
          onClick={addQuestion}
      ...
      {questions.length === 0 ? (
        <p className="text-sm text-gray-400 py-4 text-center">No questions yet. Generate them from the job description or add manually.</p>
      ) : (
            ...
            return (
              <div key={cat}>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-[11px] text-gray-400">{items.length} questions</span>
                </div>
                <div className="space-y-2">
                  {items.map(({ originalIndex: i, ...q }) => (
                    <div key={i} className="border border-gray-200 rounded-lg overflow-hidden">
                      ...
                      {expandedIdx === i && (
                        <div className="px-3 pb-3 space-y-2 border-t border-gray-100 animate-scale-in">
                          <input
                            value={q.question}
                            onChange={(e) => updateQuestion(i, "question", e.target.value)}
                            className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm font-medium mt-2 shadow-xs focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition-all"
