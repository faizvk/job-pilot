  ...
  if (!parsed) {
    return (
      <div className="border border-slate-200 rounded-xl p-5 text-center">
        <GithubIcon className="w-8 h-8 text-slate-300 mx-auto mb-2" />
        <p className="text-sm text-slate-500">Add your GitHub username in the profile above to see stats</p>
      </div>
    );
  }
    ...
    <div className="border border-slate-200 rounded-xl p-5 space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-slate-900">GitHub Stats</h3>
          <span className="text-xs text-slate-400">@{parsed}</span>
        </div>
        <button
          onClick={fetchStats}
          disabled={loading}
          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium bg-slate-900 text-white hover:bg-slate-800 disabled:opacity-50 transition-all active:scale-[0.98]"
        >
      ...
      {data && (
        <>
          ...
          {Object.keys(data.languages).length > 0 && (
            <div>
              <h4 className="text-xs font-semibold text-slate-600 mb-2">Top Languages</h4>
              <div className="flex flex-wrap gap-1.5">
                {Object.entries(data.languages)
                  .sort((a, b) => b[1] - a[1])
                  .slice(0, 8)
                  .map(([lang, count]) => (
                    <span key={lang} className="inline-flex items-center gap-1 text-xs px-2 py-1 bg-slate-50 rounded-full text-slate-700">
            ...
            <div className="space-y-2">
              {data.repos.slice(0, 6).map((repo) => (
                <div key={repo.name} className="border border-slate-100 rounded-lg p-3 hover:border-slate-200 transition-all">
                  ...
                  {repo.description && <p className="text-xs text-slate-500 mt-1 line-clamp-1">{repo.description}</p>}
