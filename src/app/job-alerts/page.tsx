    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Automated job searches with notifications</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={handleRunAll}
            disabled={runningAll || alerts.length === 0}
            className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium border border-gray-200 text-gray-600 hover:bg-gray-50 disabled:opacity-50 transition-all"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 bg-indigo-600 text-white px-4 py-2 rounded-lg text-xs font-medium hover:bg-indigo-700 transition-all active:scale-[0.98]"
          >
      ...
      {showForm && (
        <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-4">
          <h3 className="text-sm font-semibold text-gray-900">Create Job Alert</h3>
          <div className="grid grid-cols-2 gap-3">
            <input
              placeholder="Alert name (e.g. React Jobs)"
              value={form.name}
              onChange={(e) => setForm({ ...form, name: e.target.value })}
              className="col-span-2 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400"
            />
            <input
            ...
            <input
              ...
              className="border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400"
            />
            <select
          ...
          <div className="flex justify-end gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 bg-indigo-600 text-white px-4 py-1.5 rounded-lg text-xs font-medium hover:bg-indigo-700 disabled:opacity-50 transition-all"
            >
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white border border-gray-200 rounded-xl">
          ...
          <p className="text-sm text-gray-500">No job alerts yet</p>
          <p className="text-xs text-gray-400 mt-1">Create one to start getting notified about new jobs</p>
        </div>
      ) : (
        <div className="space-y-3">
          {alerts.map((alert) => (
            <div key={alert.id} className={`bg-white border rounded-xl p-4 flex items-center justify-between transition-all ${alert.isActive ? "border-gray-200" : "border-gray-100 opacity-60"}`}>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${alert.isActive ? "bg-emerald-50 text-emerald-600" : "bg-gray-100 text-gray-400"}`}>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <button
                  ...
                  className={`px-2 py-1 rounded-lg text-[10px] font-medium transition-all ${alert.isActive ? "bg-gray-100 text-gray-600 hover:bg-amber-50 hover:text-amber-600" : "bg-emerald-50 text-emerald-600 hover:bg-emerald-100"}`}
                >
