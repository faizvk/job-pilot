...
export default function FollowUpsPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {creating && (
        <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-xs animate-scale-in">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-[14px] text-gray-900">Create Follow-up</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            <div className="md:col-span-2">
              <label className="block text-[11px] font-medium text-gray-500 mb-1">Application</label>
              <select
                value={createForm.applicationId}
                onChange={(e) => setCreateForm({ ...createForm, applicationId: e.target.value })}
                className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm shadow-xs focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400"
              >
          ...
          <p className="text-xs text-gray-400 mt-2">An email draft will be auto-generated based on the follow-up type.</p>
          <div className="flex justify-end mt-4 gap-2">
            <button
              onClick={() => setCreating(false)}
              className="border border-gray-200 px-3 py-1.5 rounded-lg text-sm text-gray-600 hover:bg-gray-50 shadow-xs transition-all"
            >
              Cancel
            </button>
            <button
              ...
              className="bg-indigo-600 text-white px-4 py-1.5 rounded-lg text-sm font-medium hover:bg-indigo-700 disabled:opacity-50 shadow-sm transition-all active:scale-[0.98]"
            >
      ...
      {loading ? (
        <div className="space-y-3">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-20 bg-white border border-gray-200 rounded-xl animate-shimmer" />
          ))}
        </div>
      ) : followUps.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-sm text-gray-400 mt-1">Create one to stay on top of your applications</p>
        </div>
      ) : (
